...
export default function AlertsPage() {
  ...
  const fetchAlerts = async (): Promise<void> => {
    try {
      ...
      setError(
        'Could not retrieve active job alerts registry.'
      );
 ...
 const handleDeleteAlert = async (id: number) => {
    try {
      ...
      if (Array.isArray(updatedAlerts)) {
        ...
        // Fallback: instantly remove the deleted item from the local state
        setAlerts(prev => prev.filter(item => item.id !== id));
      }
    } catch (e: any) {
      console.error("Error deleting job alert:", e.response?.data || e.message);
    }
  };

  const handleDeleteSkill = handleDeleteAlert; // Alias for safety
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 py-10 px-4 sm:px-6 lg:px-8 font-sans selection:bg-indigo-500 selection:text-white w-full relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto space-y-8 relative z-10">
        ...
        {/* Toast Notification */}
        {showToast && (
          <div className="fixed bottom-6 right-6 bg-slate-900/90 backdrop-blur-xl border border-emerald-500/30 text-white px-5 py-4 rounded-2xl shadow-2xl shadow-emerald-950/50 flex items-center gap-3 text-sm font-medium z-50 animate-in fade-in slide-in-from-bottom-4">

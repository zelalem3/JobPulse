...
import { 
  ...
  MapPin, Briefcase, DollarSign, Share2, ShieldAlert, Database, Loader2, Award 
} from 'lucide-react';
            ...
            {job.skills && job.skills.length > 0 && (
              <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm space-y-3">
                <h3 className="text-xs font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                  <Award size={14} className="text-blue-600" /> Required Skills & Stack
                </h3>
                <div className="flex flex-wrap gap-2">
                  {job.skills.map((skill) => (
                    <span 
                      key={skill.id} 
                      className="px-3 py-1 bg-slate-50 border border-slate-200/80 text-slate-700 font-semibold text-xs rounded-xl"
                    >
                      {skill.name}
                    </span>
                  ))}

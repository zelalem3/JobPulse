...
class EthiNGOJobsScraper(BaseScraper):
    def __init__(self):
        super().__init__("EthiNGOJobs")
        self.base_url = "https://www.ethiongojobs.com"
        self.headers = {
    ...
    def fetch(self) -> list:
        ...
        try:
            print(f"[EthiNGOJobs] Fetching listings from {self.base_url}...")
            response = requests.get(self.base_url, headers=self.headers, impersonate="chrome", timeout=30)
            if response.status_code != 200:
                print(f"[EthiNGOJobs] Failed to fetch home index, status code: {response.status_code}")
                return []
            ...
            for link in soup.find_all("a", href=True):
                ...
                # EthiNGOJobs listing cards typically point to internal article paths or absolute links
                ...
                # Filter criteria to target individual job pages and skip category/tag archives or homepage
                if clean_url.rstrip("/") != self.base_url.rstrip("/"):
                    if "tag" not in clean_url and "category" not in clean_url and "page" not in clean_url:
                        job_links.add(clean_url)

            print(f"[EthiNGOJobs] Found {len(job_links)} unique job links.")
        except Exception as e:
            print(f"[EthiNGOJobs] Fetch error: {e}")
    ...
    def parse(self, url: str) -> JobListing | None:
        try:
            ...
            if response.status_code != 200:
                print(f"[EthiNGOJobs] Failed to fetch job page {url}: {response.status_code}")
                return None
            ...
            if description and ("REQUIREMENT" in description.upper() or "QUALIFICATION" in description.upper() or "RESPONSIBILITY" in description.upper()):
                for keyword in ["REQUIREMENT", "QUALIFICATION", "JOB REQUIREMENT"]:
            ...
            print(f"[EthiNGOJobs] Error parsing {url}: {e}")
            return None
    ...
    async def run(self):
        ...
        print(f"[EthiNGOJobs] Finished. Total successfully parsed: {len(jobs)}")
        return jobs